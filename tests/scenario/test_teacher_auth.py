...
def test_teaher_auth(user1_auth_hearders):
    ...
    assert create_teacher_fail.status_code == 401, f"Create teacher passed without auth"
    ...
    # проверяем, что статус код 401
    assert get_teacher_fail.status_code == 401, f"Get teacher passed without auth"
    # GETITEMS
    ...
    # проверяем, что статус код 401
    assert getitems_teacher_fail.status_code == 401, f"GetItems teacher passed without auth"
    # UPDATE
    ...
    # проверяем, что статус код 401
    assert update_teacher_fail.status_code == 401, f"Update teacher passed without auth"
    # DELETE
    ...
    # проверяем, что статус код 401
    assert delete_teacher_fail.status_code == 401, f"Delete teacher passed without auth"
